...
router.get('/all-bookings', protect, async (req, res) => {
  ...
  router.delete('/delete-booking/:bookingId', protect, async (req, res) => {
    ...
    try {
      ...
      // Ambil kuota terkait dari booking
      const kuota = await Kuota.findById(booking.kuotaId);
      if (kuota) {
        // Kembalikan kuota setelah booking dihapus
        kuota.sisa_kuota += booking.numberOfTickets;
        await kuota.save();
      }
